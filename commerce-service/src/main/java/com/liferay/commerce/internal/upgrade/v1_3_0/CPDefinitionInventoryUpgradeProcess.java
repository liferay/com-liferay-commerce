...
package com.liferay.commerce.internal.upgrade.v1_3_0;
...
public class CPDefinitionInventoryUpgradeProcess extends UpgradeProcess {
	...
	public CPDefinitionInventoryUpgradeProcess(
	...
	protected void doUpgrade() throws Exception {
		_addColumn(
			CPDefinitionInventoryModelImpl.class,
			CPDefinitionInventoryModelImpl.TABLE_NAME, "CProductId", "LONG");

		_addIndexes(CPDefinitionInventoryModelImpl.TABLE_NAME);
		...
		try {
			ps = connection.prepareStatement(
				"update CPDefinitionInventory set CProductId = ? where " +
					"CPDefinitionId = ?");

			s = connection.createStatement();

			rs = s.executeQuery("select * from CPDefinitionInventory");
		...
		runSQL("drop index IX_34D62DF1 on CPDefinitionInventory");

		runSQL("alter table CPDefinitionInventory drop column CPDefinitionId");
	}
	...
	private static final Log _log = LogFactoryUtil.getLog(
		CPDefinitionInventoryUpgradeProcess.class);
