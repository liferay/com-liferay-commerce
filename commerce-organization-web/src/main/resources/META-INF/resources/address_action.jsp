...
>
	<c:if test="<%= commerceOrganizationAddressesDisplayContext.hasManageCommerceAddressPermission() %>">
		<liferay-ui:icon
			message="edit"
			url="<%= commerceOrganizationAddressesDisplayContext.getEditCommerceAddressHref(commerceAddress.getCommerceAddressId()) %>"
		/>

		<portlet:actionURL name="editCommerceAddress" var="deleteURL">
			<portlet:param name="<%= Constants.CMD %>" value="<%= Constants.DELETE %>" />
			<portlet:param name="redirect" value="<%= currentURL %>" />
			<portlet:param name="commerceAddressId" value="<%= String.valueOf(commerceAddress.getCommerceAddressId()) %>" />
		</portlet:actionURL>
