...
public class JobLauncherImpl implements JobLauncher {
	...
	public boolean isJobActive(String jobId) {
		return _jobExecutionMap.containsKey(Objects.requireNonNull(jobId));
	}
	...
	public JobExecution run(Job job, JobParameters jobParameters) {
		...
		JobExecution jobExecution = new JobExecution(
			new JobInstance(jobId, job.getName()), jobParameters);

		_blockingExecutor.execute(
			new JobRunnable(job, jobExecution, _jobExecutionMap));

		_jobExecutionMap.put(jobId, jobExecution);
