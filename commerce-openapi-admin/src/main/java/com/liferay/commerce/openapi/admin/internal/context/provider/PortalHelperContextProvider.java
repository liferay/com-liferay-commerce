...
@Provider
public class PortalHelperContextProvider
	implements ContextProvider<PortalHelper> {

	@Override
	public PortalHelper createContext(Message message) {
		...
		try {
			...
			if (_log.isWarnEnabled()) {
				_log.warn("Unable to fetch group by Id: " + groupId, pe);
			}
		}
		...
		try {
			company = _portal.getCompany(httpServletRequest);
		}
		catch (PortalException pe) {
			if (_log.isWarnEnabled()) {
		...
		return new PortalHelper(
			group, locale, permissionChecker, user, themeDisplay, company);
	}

	private static final Log _log = LogFactoryUtil.getLog(
		PortalHelperContextProvider.class);
